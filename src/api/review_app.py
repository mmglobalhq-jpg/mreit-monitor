...
def _get_available_periods() -> dict:
    ...
    companies = client.table("companies_ML_REIT").select("id, ticker, name").eq("is_active", True).execute().data
    ...
    monthly_rows = client.table("monthly_metrics_ML_REIT").select("company_id, as_of_date").order("as_of_date", desc=True).execute().data
    seen_monthly = set()
    ...
    quarterly_rows = client.table("quarterly_metrics_ML_REIT").select("company_id, period_end_date, quarter_label").order("period_end_date", desc=True).execute().data
    seen_quarterly = set()
    ...
    ue_rows = client.table("universal_extractions_ML_REIT").select("company_id, fiscal_year, fiscal_quarter, period_end").order("period_end", desc=True).execute().data
    for row in ue_rows:
        co = co_map.get(row["company_id"])
        if not co:
            continue
...
async def send_report_email(report_id: str):
    ...
    # Mark as sent
    client.table("summary_reports_ML_REIT").update({
        "email_sent": True,
...
async def run_ab_test(
    ...
    doc = client.table("company_documents_ML_REIT").select("*").eq("id", document_id).limit(1).execute().data
    if not doc:
        raise HTTPException(status_code=404, detail="Document not found")
    doc = doc[0]

    company = client.table("companies_ML_REIT").select("*").eq("id", doc["company_id"]).limit(1).execute().data
    if not company:
    ...
    companies_list = client.table("companies_ML_REIT").select("id, ticker").execute().data
    ...
    for model in models:
        try:
            ...
            try:
                client.table("extraction_comparisons_ML_REIT").insert({
